...
Zu Beginn des Projekts wird der einbeinige Krabbelroboter des Legacy-Projekts in Bezug auf Software, Hardware und Funktionsumfang analysiert. Basis dafür sind hauptsächlich die Dokumentationen der Vorgängerprojekte \cite{vorgängerprojekt}. 
...
Zur Messung von Drehzahlunterschieden an den Rädern werden die zwei Inkrementalgeber des Typs \textit{MEC22} der Firma PWB \cite{pwb_me16_datasheet_2011} aus dem Legacy-Projekt weiterverwendet. Mit einer Auflösung von 500 Zählimpulsen pro Umdrehung (Counts per Revolution, CPR) und einem Raddurchmesser von \qty{5}{\centi\m} entspricht ein Impuls ca. \qty{0,31}{\milli\m} zurückgelegte Strecke. Dies ermöglicht eine präzise Erfassung kleinster Bewegungen für das Reinforcement Learning. Die Stromversorgung erfolgt direkt über den Raspberry Pi.

\textbf{Die Aktoren}

Für die Beinbewegung wurden Servomotoren vom Typ \textit{Dynamixel XL430-W250-T} (Robotis) \cite{robotis_xl430} eingesetzt. Sie bieten ein gutes Verhältnis von Drehmoment (bis 1{,}5\,Nm) zu Energieverbrauch (Leerlaufstrom ca. \qty{40}{\milli\ampere}) sowie präzise Positionssteuerung und sind über eindeutige IDs adressierbar.
...
Der neu ausgewählte \textit{Gens ace Modellbau-Akkupack (LiPo)} \cite{gens_ace_akku} ist mit seinen ebenfalls \qty{7.4}{\volt} Ausgangsspannung ähnlich aufgebaut, wiegt allerdings nur noch \qty{66}{\gram} und ist deutlich kompakter. Aufgrund des geplanten Einsatzes als Demonstrationsroboter ist die damit verbundene geringere Laufzeit kein Problem. Folgende Überschlagsrechnung zur Laufzeit des Crawlers verwendet den maximal gemessenen Strom bei Bewegung der Motoren\,(\qty{1070}{\milli\ampere}), nicht den Strom bei normalem Betrieb:

\begin{align*}
t_{\text{Betrieb, alter Akku}} &= \frac{\qty{5400}{\milli\ampere\hour}}{\qty{1070}{\milli\ampere}} \approx \qty{5}{\hour} \\
t_{\text{Betrieb, neuer Akku}} &= \frac{\qty{1500}{\milli\ampere\hour}}{\qty{1070}{\milli\ampere}} \approx \qty{1.4}{\hour}
\end{align*}
...
An vorderer Stelle, sowohl in unserer Priorisierung als auch chronologisch im Projektverlauf, stand die Verbesserung der Developer Experience (DX), also eine bewusste Investition in einen reibungslosen Entwicklungsprozess, die mit einer geringeren Fehleranfälligkeit, erhöhtem Komfort und einer gesteigerten Entwicklungsgeschwindigkeit für uns und mögliche nachfolgende Teams einhergeht. 

Das Legacy-Projekt verfolgte dies bereits mit einem teilweisen Umstieg von Versionskennzeichnungen im Dateinamen auf eine übliche Versionskontrolle mit Git. Das führten wir weiter und nutzten GitHub zur einfachen Kollaboration. 

\textbf{Synchronisierung des Quellcodes}

Im Legacy-Projekt wurde der Programmcode direkt auf dem Raspberry Pi entwickelt, der den Roboter steuert. Wir wollten es dagegen ermöglichen, den Code lokal etwa auf dem eigenen Laptop zu editieren. Die Synchronisierung des Codes vom Endgerät auf den Pi erfolgt via SSH oder FTP, wobei \texttt{rsync} für eine beschleunigte Synchronisierung durch das Überspringen unveränderter Dateien genutzt wird. Es muss dafür eine TCP-Verbindung zum Raspberry Pi bestehen, z.\, B. durch ein lokales Netzwerk, unterwegs durch einen mobilen Hotspot oder auch über das Internet durch eine Portfreigabe. Eine weitere Option ist die Nutzung eines SSH-Reverse-Tunnels über einen öffentlichen Anbieter, sodass kein direktes Routing zum Pi ermöglicht werden muss. 

Der Pi muss somit niemals zwingend über einen Bildschirm verfügen, da das Ausführen der ROS-Startbefehle via SSH und der Zugriff auf das Webinterface via HTTP von einem externen Gerät aus geschieht (gleichzeitig kann natürlich jederzeit auch stattdessen ein Bildschirm bzw. weitere Peripherie direkt angeschlossen werden). Da der Code lokal editiert wird, kann nun auch parallel gearbeitet werden, und dabei eine vollwertige Entwicklungsumgebung und andere Tools verwendet werden, für die die Leistung des Raspberry Pi nicht ausreicht. Ein Teil der Crawler-Software, das Webinterface, kann sogar ohne Probleme lokal statt auf dem Pi gebaut werden, was die Build-Zeiten merklich verkürzt. 
...
Der Entwicklungszyklus besteht aus mehreren Schritten, die notwendig sind, um das Programm auf dem Roboter auszuführen, darunter etwa die Synchronisierung des Quellcodes auf den Raspberry Pi oder das korrekte Starten mit einem ROS-Befehl. Mit Pixi kann dieser mehrschrittige Prozess vereinfacht werden, indem die Schritte in komfortablen Pixi-Befehlen wie \texttt{pixi run upload} oder \texttt{pixi run build-web} definiert sind. Dadurch ist es im Vergleich zum Vorgängerprojekt leichter, den Build-Prozess zu dokumentieren. 

\textbf{Python Type Hints}

Ein weiterer Schritt in der Verbesserung der Developer Experience war das Hinzufügen sogenannter Type Hints in der Python-Codebase. Als Programmiersprache mit dynamischen Typen passieren in Python schnell Fehler, wenn Variablen bezüglich ihres Typs falsch verwendet werden. Diese können jedoch erst während der Ausführung des Programms entdeckt werden. Type Hints können in einer Codebase hinzugefügt werden, um diese Art der Fehler bereits vorher automatisch zu identifizieren. 

Im einem simplen Beispiel sähen Type Hints folgendermaßen aus:
...
Die hinzugefügten Type Hints geben an, dass die Variable \texttt{s} vom Typ String sein muss, sowie, dass die \texttt{greet}-Funktion einen String als Output produziert. Statt eines \texttt{TypeError} zur Laufzeit wird nun eine entsprechende Warnung im Codeeditor angezeigt. 
...
\begin{figure}[h]
	\includegraphics[width=\textwidth]{photos/crawler_architecture_portrait.png}
	\caption{Internes Diagramm der Architektur der ROS-Nodes}
  \label{fig:crawler_architecture}
\end{figure}

\textbf{Hardware-Nodes}

% Gekürzt und Ergebnisse auslagert für besseres Ergebnis-Kapitel
...
Gemäß dem Prinzip der Modularität wird jede Hardwarekomponente (Motoren, Inkrementalgeber, etc.) durch eine eigene ROS-Node gesteuert. Für ein Proof of Concepts setzten wir als Prototyp eine LED ("Blinker") ein, die über einen GPIO-Pin angesteuert und die Node \texttt{crawler\_blinker} verwaltet wird. Der Pin der LED wird dabei als flexibler Parameter in der ROS-Launchdatei definiert. Ein entscheidenes Merkmal ist die softwareseitige Abstraktion: Über die Umgebungsvariablen kann zwischen der physischen Ansteuerung und einer Hardware-Mock-Implementation gewechselt werden, was die Entwicklung des Webinterfaces ohne Roboter-Hardware ermöglicht. Die Kommunikation erfolgt über eine standardisierte Topics: Subscriptions (z.B. \texttt{/toggle} oder \texttt{/write}) nehmen Steuerbefehle entgegen, während der aktuelle Zustand kontinuierlich auf einem \texttt{/state}-Topic veröffentlicht wird. Nach diesem einheitlichen Schema arbeiten auch die Nodes für die Motoren (\texttt{crawler\_motors}) und Sensoren (\texttt{crawler\_encoders}).

\textbf{Web-API-Node}

% Gekürzt und Ergebnisse ausgelagert für besseres Ergebnis-Kapitel, hier: zu sehr gekürzt?
...
Als Schnittstelle zwischen dem ROS-Backend und dem Webinterface dient die Node \texttt{crawler\_web\_api}. Sie stellt HTTP-Endpoints (bspw. \texttt{/api/manuel/blinker/toggle}) bereit, die eingehende Befehle direkt als ROS-Message an die Hardware-Nodes (s. oben) weiterleiten. Für den umgekehrten Datenfluss vom Roboter zum Webinterface werden WebSockets genutzt. Im Gegensatz zum Legacy-Projekt, bei dem die WebSockets lediglich als Trigger für erneute HTTP-Anfragen vom Client dienten, werden nun Rohdaten direkt im numerischen Format über die Verbindung übertragen. Um die Netzwerklast und die Anzahl der Verbindungen zu minimieren, aggregiert die Node die Datenströme verschiedener Topics (z.B. Motorposition und Sensorwerte) in einer zentralen Datenstruktur, die bei jeder Zustandsänderung in Echtzeit an das Webinterface, also den Client, gesendet wird. Dies ermöglicht eine dynamische Visualisierung der Hardwarezustände im Webinterface ohne zusätzliche Latenzen. 
...
\textbf{RL-Environment-Node:} Der Teil der Anwendung, der für das Reinforcement Learning zuständig ist (siehe Abschnitt~\ref{sec:reinforcement_learning}), besitzt ebenso eine sehr modulare Struktur. Für die Überwachung und Verwaltung des Reinforcement-Learning-Prozesses gibt es etwa die Node \texttt{crawler\_rl\_environment}. Diese übersetzt, entsprechend ihrer Benennung, die über die Topics gelesenen Hardwarezustände in ein für den RL-Algorithmus passendes Format eines Environments im Sinne des RL-Frameworks; ebenso bestimmt sie den Reward anhand der Inkrementalgeber-Daten. Der Reinforcement-Learning-Zyklus wird von der Environment-Node ausgehend gesteuert: Nachdem sie eine Nachricht zum Starten empfangen hat, sendet sie das erste Environment (ohne Reward) auf \texttt{/crawler/rl/state\_reward} und wartet auf die Antwort der RL-Node auf \texttt{/crawler/rl/action}. Entsprechend der gewünschten Aktion werden Nachrichten zur Hardwaresteuerung gesendet, und das nächste Environment und der nächste Reward werden in einem fortlaufenden Prozess gesendet.
...
\textbf{RL-Node:} In dieser Node ist der eigentliche Reinforcement-Learning-Algorithmus implementiert. Sie empfängt die Zustände und Rewards und sendet die daraus resultierende Aktion an die Environment-Node (s. oben). Durch die abstrakte Schnittstellt bleibt die RL-Node modular und kann bei Bedarf, z.B. über das Webinterface, durch alternative Algorithmen ersetzt werden. Die Node wird bei Bedarf mit den gewünschten Parametern über das Webinterface gestartet.
...
\begin{figure}[h]
	\includegraphics[width=\textwidth]{photos/webinterface_rlControl_2_updated_logo.png}
	\caption{RL Control im Webinterface nach dem Starten, mit Visualisierungen des RL-Environments (links) und der Q-Table (rechts)}
\end{figure}
...
Da der Crawler als Demonstrationsobjekt dient, wurde das Webinterface von Grund auf neu entwickelt, um eine flüssige Benutzererfahrung zu gewährleisten. Es umfasst zwei Hauptbereiche: 

\begin{enumerate}

  \item \textbf{Manual Control} ermöglicht die manuelle Steuerung der Motoren und visualisiert die Daten der Inkrementalgeber. Die UI-Elemente sind physischen Komponenten nachempfunden, während Animationen und farbliche Hervorhebungen die Interaktionen und Zustände in Echtzeit verdeutlichen. 

  \item \textbf{RL Control} bietet eine Oberfläche zur Konfiguration und Überwachung des Reinforcement Learning. Neben dem aktuellen Environment-Zustand und dem Reward zeigt ein dynamisches Diagramm den Lernfortschritt (Zeit vs. zurückgelegte Strecke) an. Die interne Q-Table wird als Heatmap visualisiert, wobei Zell-Aktualisierungen durch kurze Highlights sichtbar gemacht werden. Zusätzlich ermöglicht die Oberfläche manuelle Eingriffe in die Aktionswahl des Q-Learning-Algorithmus, um den Lernprozess schrittweise nachvollziehen zu können. Zudem ermöglicht das Webinterface den Export und Import der Q-Table in einem serialisierten Textformat, um erfolgreiche Lernvorgänge zu speichern und wiederzuverwenden.

\end{enumerate}

Technisch gesehen basiert das Backend auf dem Python-Webframework \emph{Flask} \cite{flask}, während das Frontend mit \emph{React} \cite{react} umgesetzt wurde. Dieses erleichert durch seinen komponentenbasierten Ansatz die Echtzeit-Datenanzeige. Für das Design wurde \emph{Tailwind CSS} \cite{tailwindcss} genutzt, während \emph{Chart.js} \cite{chartjs} die dynamischen Diagramme rendert. 
