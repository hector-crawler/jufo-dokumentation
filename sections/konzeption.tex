...
Gemäß dem Prinzip der Modularität wird jede Hardwarekomponente (Motoren, Inkrementalgeber, etc.) durch eine eigene ROS-Node gesteuert. Für ein Proof of Concepts setzten wir als Prototyp eine LED ("Blinker") ein, die über einen GPIO angesteuert und die Node \texttt{crawler\_blinker} verwaltet wird. Der PIN der LED wird dabei als flexibler Parameter in der ROS-Launchdatei definiert. Ein entscheidenes Merkmal ist die softwareseitige Abstraktion: Über die Umgebungsvariablen kann zwischen der physischen Ansteuerung und einer Hardware-Mock-Implemnetation gewechselt werden, was die Entwicklung des Webinterfaces ohne Roboter-Hardware ermöglicht. Die Kommunikation erfolgt über eine standardisierte Topics: Subscriptions (z.B. \texttt{/toggle} oder \texttt{/write}) nehmen Steuerbefehle entgegen, während der aktuelle Zustand kontinuierlich auf einem \texttt{state}-Topic veröffentlicht wird. Nach diesem einheitlichen Schema arbeiten auch die Nodes für die Motoren (\texttt{crawler\_motors}) und Sensoren (\texttt{crawler\_encoders}).
